...
private extension View {
    func applyNavigationStackLayer4Modifiers() -> some View {
        self
            .automaticCompliance(named: "platformImplementNavigationStack_L4")
...
public func platformImplementNavigationStack_L4<Content: View>(
    ...
    guard let implementation = strategy.implementation else {
        ...
        case .navigationStack:
            return createNavigationStackView(content: contentWithTitle)
        
        case .navigationView:
            return createNavigationViewView(content: contentWithTitle)
        
        case .splitView:
            return createNavigationSplitView(content: contentWithTitle)
        
        case .modal:
            return createModalView(content: contentWithTitle)
        }
}
...
private func createNavigationStackView<Content: View>(content: Content) -> AnyView {
    ...
    if #available(iOS 16.0, *) {
        return AnyView(
            ...
            .applyNavigationStackLayer4Modifiers()
        )
    } else {
        // Fallback to NavigationView
        return createNavigationViewView(content: content, useStackStyle: true)
    }
    #else
    // macOS and other platforms - use NavigationView
    return createNavigationViewView(content: content)
    #endif
}
...
private func createNavigationViewView<Content: View>(
    ...
    return AnyView(baseView.applyNavigationStackLayer4Modifiers())
}
...
private func createModalView<Content: View>(content: Content) -> AnyView {
    ...
    return AnyView(content.applyNavigationStackLayer4Modifiers())
}
