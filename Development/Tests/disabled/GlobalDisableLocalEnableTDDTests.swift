...
@MainActor
final class GlobalDisableLocalEnableTDDTests: XCTestCase {
    ...
    func testFrameworkComponentGlobalDisableLocalEnableGeneratesID() {
        ...
        XCTAssertFalse(id.isEmpty, "Framework component with local enable should generate ID")
        XCTAssertTrue(id.contains("TestApp"), "ID should contain namespace")
        
        print("🔍 Framework Component Test: Generated ID='\(id)'")
    }
    
    func testGlobalEnableLocalDisableDoesNotGenerateID() {
    ...
    func testFrameworkComponentsRespectGlobalConfig() {
        ...
        XCTAssertTrue(id.isEmpty, "Framework components should respect global config")
        
        print("🟢 TDD Green Phase: Generated ID='\(id)' - Should be empty (framework respects global)")
    }
    
    func testPlainSwiftUIRequiresExplicitEnable() {
        ...
        // This assertion SHOULD PASS
        XCTAssertTrue(id.isEmpty, "Plain SwiftUI should require explicit enable")
