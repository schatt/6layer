...
@MainActor
final class MetalRenderingCrashTDDTests: XCTestCase {
    ...
    func testPlatformPresentItemCollectionL1DoesNotCrash() {
        ...
        XCTAssertNotNil(view, "platformPresentItemCollection_L1 should not crash")
        
        // Try to inspect the view (should not crash)
        do {
            let inspectedView = try view.inspect()
            XCTAssertNotNil(inspectedView, "View should be inspectable without crashing")
            print("✅ platformPresentItemCollection_L1 rendered successfully")
        } catch {
            XCTFail("platformPresentItemCollection_L1 should not crash during inspection: \(error)")
        }
        
        print("🟢 TDD Green Phase: platformPresentItemCollection_L1 no longer crashes - performance layer removed")
    }
    
    func testGenericItemCollectionViewDoesNotCrash() {
        // TDD Green Phase: Performance layer removed, so no Metal crash
        ...
        XCTAssertNotNil(view, "GenericItemCollectionView should not crash")
        ...
        do {
            ...
            XCTFail("GenericItemCollectionView should not crash during inspection: \(error)")
        }
        
        print("🟢 TDD Green Phase: GenericItemCollectionView no longer crashes - performance layer removed")
    }
    
    func testMetalRenderingCrashReproduction() {
        // TDD Green Phase: Performance layer removed, so no Metal crash
        ...
        // This should NOT crash - performance layer removed
        XCTAssertNotNil(view, "Metal rendering should not crash")
        ...
        do {
            ...
            XCTFail("Metal rendering should not crash during inspection: \(error)")
        }
        
        print("🟢 TDD Green Phase: Metal rendering crash fixed - performance layer removed")
    }
    
    func testSimpleCardComponentWithRegularMaterialCrashes() {
        // TDD Green Phase: Performance layer removed, so no Metal crash
        ...
        XCTAssertNotNil(view, "SimpleCardComponent should not crash")
        
        // Try to inspect the view (should not crash)
        do {
            let inspectedView = try view.inspect()
            XCTAssertNotNil(inspectedView, "SimpleCardComponent should be inspectable without crashing")
            print("✅ SimpleCardComponent rendered successfully")
        } catch {
            XCTFail("SimpleCardComponent should not crash during inspection: \(error)")
        }
        
        print("🟢 TDD Green Phase: SimpleCardComponent no longer crashes - performance layer removed")
    }
    
    func testPerformanceLayerRemoval() {
