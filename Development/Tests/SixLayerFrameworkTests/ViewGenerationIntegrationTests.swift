...
@MainActor
final class ViewGenerationIntegrationTests: XCTestCase {
    ...
    private func testViewGenerationBehavior(_ config: CardExpansionPlatformConfig, configName: String) {
        ...
        if config.supportsTouch {
            XCTAssertTrue(config.supportsTouch, "Touch should be supported for view generation in \(configName)")
        ...
        if config.supportsHover {
            XCTAssertTrue(config.supportsHover, "Hover should be supported for view generation in \(configName)")
            XCTAssertGreaterThanOrEqual(config.hoverDelay, 0,
                                      "Hover delay should be set for \(configName)")
        }
        
        // Test accessibility behavior
        XCTAssertTrue(config.supportsVoiceOver, "VoiceOver should be supported for view generation in \(configName)")
        XCTAssertTrue(config.supportsSwitchControl, "Switch Control should be supported for view generation in \(configName)")
    }
    
    // MARK: - Individual Capability View Tests
    
    /// Test touch view generation in both enabled and disabled states
    func testTouchViewGenerationBothStates() {
        // Test touch enabled
    ...
    func testHoverViewGenerationBothStates() {
        // Test hover enabled
