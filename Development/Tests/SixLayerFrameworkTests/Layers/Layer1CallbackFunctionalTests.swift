...
open class Layer1CallbackFunctionalTests {
    ...
    @Test func testPlatformPresentItemCollectionL1WithEnhancedHintsCallbacks() async throws {
        ...
        let inspectionResult = withInspectedView(view) { inspector in
            let listViews = try inspector.sixLayerFindAll(ListCollectionView<TestItem>.self)

            if let firstList = listViews.first {
                #expect(listViews.count > 0, "Should have list view")

                // Try to find and tap a card
                let listCards = try firstList.sixLayerFindAll(ListCardComponent<TestItem>.self)
                if let firstCard = listCards.first {
                    ...
                    let hStacks = try vStack.sixLayerFindAll(HStack<Text>.self)
