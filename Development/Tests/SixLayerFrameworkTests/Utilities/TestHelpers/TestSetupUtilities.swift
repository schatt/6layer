...
public final class TestSetupUtilities {
    ...
    public func assertCardExpansionConfig(
        ...
        if let touch = touch {
            #expect(config.supportsTouch == touch, "Card config touch support should be \(touch)", )
        }
        if let haptic = haptic {
            #expect(config.supportsHapticFeedback == haptic, "Card config haptic support should be \(haptic)", )
        }
        if let hover = hover {
            #expect(config.supportsHover == hover, "Card config hover support should be \(hover)", )
        }
        if let voiceOver = voiceOver {
            #expect(config.supportsVoiceOver == voiceOver, "Card config VoiceOver support should be \(voiceOver)", )
        }
        if let switchControl = switchControl {
            #expect(config.supportsSwitchControl == switchControl, "Card config Switch Control support should be \(switchControl)", )
        }
        if let assistiveTouch = assistiveTouch {
